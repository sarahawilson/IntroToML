...
class SARSAHelper:
    ...
    def runTrain(self, epsilon, learningRate, discount, iterations, movesAllowed):
        ...
        for iteration in range(iterations):
            ...
            # Chose an action to take right away using Epsilon Greedy
            ...
            while ((not reachedFinishLine) and (loopIterations < movesAllowed)):
                ...
                # Use First Action to Deteremine S'
                ...
                if (not reachedFinishLine):
                    ...
                    #Pick Next Action
                    #Let the greedy epsilon algorithm pick the acceleration to use
                    ep_picked_acceleration_round2 = self._greedy_epsilon(q_s_prime, epsilon)
                    
                    q_s_prime_a_prime = q_s_prime[ep_picked_acceleration_round2]
                    
                    
                    # Update the q_s_a values
                    reward = -1
                    cur_q_a_values_basedOnCarState[accToApply] += (learningRate *(reward + (discount * q_s_prime_a_prime) - q_s_a_value))
                
                loopIterations = loopIterations + 1
                #epsilon = epsilon * 0.999999999
    ...
    def plotLearningCurve(self, metricsList, trackName):
        ...
        plotTitle = 'SARSA: ' + nameTrack + crashType
        plt.title(plotTitle)
    ...
    def dumpResultsToFile(self, trackName):
        ...
        fileName = '\SARSA' + crashType + nameTrack + '.txt'
